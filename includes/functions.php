 ...
 *
 * BP Dokumente are associated with groups and users through a taxonomy called
...
function bp_docs_get_item_term_id( $item_id, $item_type, $item_name = '' ) {
	...
	if ( empty( $item_term ) ) {
		...
		$item_term_args = apply_filters( 'bp_docs_item_term_values', array(
			'description' => sprintf( _x( 'Mit %1$s %2$s verknüpfte Dokumente', 'Beschreibung für den zugehörigen Taxonomiebegriff. Von der Form "Mit dem [item-type] [item-name]" - Elementtyp ist Gruppe, Benutzer usw.', 'bp-docs' ), $item_type, $item_name ),
			'slug'        => $item_term_slug,
		) );
	...
	if ( ! empty( $settings ) || $is_new_doc ) {
		...
		foreach ( $verified_settings as $verified_setting_name => $verified_setting ) {
			...
			if ( $verified_setting['verified_value'] != $verified_setting['original_value'] ) {
				$message = __( 'Dein Dokument wurde erfolgreich gespeichert, aber einige Deiner Zugriffseinstellungen wurden geändert, um den Berechtigungen des Dokuments zu entsprechen.', 'bp-docs' );
			}
		}
...
function bp_docs_get_docs_directory_title() {
	...
	if ( empty( $title ) ) {
		$title = __( 'Dokumentenverzeichnis', 'bp-docs' );
	}
