...
/**
 * Initiates a BP Dokumente query
 *
 * @since 1.2
 */
...
/**
 * Determine whether you are viewing a BP Dokumente page
 *
 * @since 1.0-beta
 *
 * @return bool
...
function bp_docs_info_header() {
	...
	function bp_docs_get_info_header() {
		...
		if ( empty( $filters ) ) {
			$message = __( 'Du siehst <strong>alle</strong> Dokumente.', 'bp-docs' );
		} else {
			$message = array();
			...
			$message .= ' - ' . sprintf( __( '<strong><a href="%s" title="Alle Dokumente anzeigen">Alle Dokumente anzeigen</a></strong>', 'bp-docs' ), $view_all_url );
		}
		...
		<?php if ( $filter_titles = bp_docs_filter_titles() ) : ?>
			<div class="docs-filters">
				<p id="docs-filter-meta">
					<?php printf( __( 'Filtern nach: %s', 'bp-docs' ), $filter_titles ) ?>
				</p>
...
function bp_docs_search_term_filter_text( $message, $filters ) {
	if ( !empty( $filters['search_terms'] ) ) {
		$message[] = sprintf( __( 'Du suchst nach Dokumenten, die den Begriff <em>%s</em> enthalten', 'bp-docs' ), esc_html( $filters['search_terms'] ) );
	}
...
function bp_docs_mygroups_link() {
	...
	/**
         * Get the link the Meine Gruppen tab of the Docs archive
         *
         * @since 1.2
         */
...
function bp_docs_mydocs_started_link() {
	...
	/**
         * Get the link to the Von mir begonnen tab of the logged in user
         *
         * @since 1.2
         */
...
function bp_docs_mydocs_edited_link() {
	...
	/**
         * Get the link to the Von mir bearbeitet tab of the logged in user
         *
         * @since 1.2
         */
...
function bp_docs_associated_group_dropdown( $args = array() ) {
	...
	if ( $r['null_option'] ) {
		$html .= '<option value="">' . __( 'Keine', 'bp-docs' ) . '</option>';
	}
...
function bp_docs_doc_associated_group_markup() {
	...
	<tr>
		<td class="desc-column">
			<label for="associated_group_id"><?php _e( 'Mit welcher Gruppe soll dieses Dokument verbunden sein?', 'bp-docs' ) ?></label>
			<span class="description"><?php _e( '(Optional) Beachte, dass die für dieses Dokument verfügbaren Zugriffseinstellungen möglicherweise durch die Privatsphäreeinstellungen der von Dir ausgewählten Gruppe eingeschränkt sind.', 'bp-docs' ) ?></span>
		</td>
	...
	if ( $group_id ) {
		...
		if ( ! empty( $group->name ) ) {
			...
			if ( 1 === $_count ) {
				...
				$group_member_count = sprintf( __( '%s Mitglied', 'bp-docs', $_count ), number_format_i18n( $_count ) );
			} else {
				$group_member_count = sprintf( _n( '%s Mitglied', '%s Mitglieder', $_count, 'bp-docs' ), number_format_i18n( $_count ) );
			}

			switch ( $group->status ) {
				case 'public' :
					$group_type_string = __( 'Öffentliche Gruppe', 'bp-docs' );
					break;

				case 'private' :
					$group_type_string = __( 'Private Gruppe', 'bp-docs' );
					break;

				case 'hidden' :
					$group_type_string = __( 'Versteckte Gruppe', 'bp-docs' );
					break;
...
function bp_docs_doc_settings_markup( $doc_id = 0, $group_id = 0 ) {
	...
	$settings_fields = array(
		'read' => array(
			'name'  => 'read',
			'label' => __( 'Wer kann dieses Dokument lesen?', 'bp-docs' )
		),
		'edit' => array(
			'name'  => 'edit',
			'label' => __( 'Wer kann dieses Dokument bearbeiten?', 'bp-docs' )
		),
		'read_comments' => array(
			'name'  => 'read_comments',
			'label' => __( 'Wer kann Kommentare zu diesem Dokument lesen?', 'bp-docs' )
		),
		'post_comments' => array(
			'name'  => 'post_comments',
			'label' => __( 'Wer kann Kommentare zu diesem Dokument veröffentlichen?', 'bp-docs' )
		),
		'view_history' => array(
			'name'  => 'view_history',
			'label' => __( 'Wer kann den Verlauf dieses Dokuments anzeigen?', 'bp-docs' )
		)
	);
...
function bp_docs_doc_action_links() {
	...
	$links[] = '<a href="' . bp_docs_get_doc_link() . '">' . __( 'Lesen', 'bp-docs' ) . '</a>';

	if ( current_user_can( 'bp_docs_edit', get_the_ID() ) ) {
		$links[] = '<a href="' . bp_docs_get_doc_edit_link() . '">' . __( 'Bearbeiten', 'bp-docs' ) . '</a>';
	}

	if ( current_user_can( 'bp_docs_view_history', get_the_ID() ) && defined( 'WP_POST_REVISIONS' ) && WP_POST_REVISIONS ) {
		$links[] = '<a href="' . bp_docs_get_doc_link() . BP_DOCS_HISTORY_SLUG . '">' . __( 'Verlauf', 'bp-docs' ) . '</a>';
	}
...
function bp_docs_delete_doc_button( $doc_id = false ) {
	...
	function bp_docs_get_delete_doc_button( $doc_id = false ) {
		...
		if ( bp_docs_is_doc_trashed( $doc_id ) ) {
			...
			$button = ' <a class="delete-doc-button untrash-doc-button confirm" href="' . bp_docs_get_remove_from_trash_link( $doc_id ) . '">' . __( 'Wiederherstellen', 'bp-docs' ) . '</a>';
			// and a button to permanently delete the doc.
			$button .= '<a class="delete-doc-button confirm" href="' . bp_docs_get_delete_doc_link() . '">' . __( 'Permanent löschen', 'bp-docs' ) . '</a>';
		} else {
			// A button to move the doc to the trash...
			$button = '<a class="delete-doc-button confirm" href="' . bp_docs_get_delete_doc_link() . '">' . __( 'Papierkorb', 'bp-docs' ) . '</a>';
			// and a button to permanently delete the doc.
			$button .= '<a class="delete-doc-button confirm" href="' . bp_docs_get_delete_doc_link( true ) . '">' . __( 'Permanent löschen', 'bp-docs' ) . '</a>';
		}
...
/**
 * Outputs the tabs at the top of the Docs view (Alle Dokumente, New Doc, etc)
 *
...
function bp_docs_create_button() {
	if ( ! bp_docs_is_doc_create() && current_user_can( 'bp_docs_create' ) ) {
		echo apply_filters( 'bp_docs_create_button', '<a class="button" id="bp-create-doc-button" href="' . bp_docs_get_create_link() . '">' . __( "Neues Dokument erstellen", 'bp-docs' ) . '</a>' );
	}
}
...
function bp_docs_doc_permissions_snapshot( $args = array() ) {
	...
	if ( bp_is_active( 'groups' ) ) {
		...
		if ( ! empty( $doc_groups ) ) {
			...
			$html .= '<span>' . __('Gruppe: ', 'bp-docs') . '</span>';
	...
	$levels = array(
		'anyone'        => __( 'Jedermann', 'bp-docs' ),
		'loggedin'      => __( 'Angemeldete Benutzer', 'bp-docs' ),
		'friends'       => __( 'Meine Freunde', 'bp-docs' ),
		'creator'       => __( 'Nur der Autor', 'bp-docs' ),
		'no-one'        => __( 'Nur ich', 'bp-docs' )
	);

	if ( bp_is_active( 'groups' ) ) {
		$levels['group-members'] = sprintf( __( 'Mitglieder von: %s', 'bp-docs' ), $group_names );
		$levels['admins-mods'] = sprintf( __( 'Administratoren und Mods der Gruppe %s', 'bp-docs' ), $group_names );
	}

	if ( get_the_author_meta( 'ID' ) == bp_loggedin_user_id() ) {
		$levels['creator'] = __( 'Nur der Autor (das bist du!)', 'bp-docs' );
	}
	...
	$read_text  = sprintf( __( 'Dieses Dokument kann gelesen werden von: <strong>%s</strong>', 'bp-docs' ), $levels[ $settings['read'] ] );

	// Edit
	$edit_class = bp_docs_get_permissions_css_class( $settings['edit'] );
	$edit_text  = sprintf( __( 'Dieses Dokument kann bearbeitet werden von: <strong>%s</strong>', 'bp-docs' ), $levels[ $settings['edit'] ] );

	// Read Comments
	$read_comments_class = bp_docs_get_permissions_css_class( $settings['read_comments'] );
	$read_comments_text  = sprintf( __( 'Kommentare sind sichtbar für: <strong>%s</strong>', 'bp-docs' ), $levels[ $settings['read_comments'] ] );

	// Post Comments
	$post_comments_class = bp_docs_get_permissions_css_class( $settings['post_comments'] );
	$post_comments_text  = sprintf( __( 'Kommentare können gepostet werden von: <strong>%s</strong>', 'bp-docs' ), $levels[ $settings['post_comments'] ] );

	// View History
	$view_history_class = bp_docs_get_permissions_css_class( $settings['view_history'] );
	$view_history_text  = sprintf( __( 'Der Verlauf kann angezeigt werden durch: <strong>%s</strong>', 'bp-docs' ), $levels[ $settings['view_history'] ] );
	...
	if ( $settings_count == $private_count ) {
		$summary       = 'private';
		$summary_label = __( 'Privat', 'bp-docs' );
	} else if ( $settings_count == $anyone_count ) {
		$summary       = 'public';
		$summary_label = __( 'Öffentlich', 'bp-docs' );
	} else {
		$summary       = 'limited';
		$summary_label = __( 'Begrenzt', 'bp-docs' );
	}

	$html .= '<div id="doc-permissions-summary" class="doc-' . $summary . '">';
	$html .= $summary_before_content;
 $html .=   sprintf( __( 'Zugriff: <strong>%s</strong>', 'bp-docs' ), $summary_label );
	$html .=   '<a href="#" class="doc-permissions-toggle" id="doc-permissions-more">' . __( 'Zeige Details', 'bp-docs' ) . '</a>';
	$html .= $summary_after_content;
 $html .= '</div>';
	...
	if ( current_user_can( 'bp_docs_manage' ) )
		$html .=   '<a href="' . bp_docs_get_doc_edit_link() . '#doc-settings" id="doc-permissions-edit">' . __( 'Bearbeiten', 'bp-docs' ) . '</a>';

	$html .=   '<a href="#" class="doc-permissions-toggle" id="doc-permissions-less">' . __( 'Details ausblenden', 'bp-docs' ) . '</a>';
	$html .= '</div>';
...
/**
 * Is this the Meine Gruppen tab of the Docs archive?
 *
 * @since 1.2
 * @return bool
 */
...
/**
 * Is this the Meine Gruppen directory?
 *
 * @since 1.5
 * @return bool
 */
...
function bp_docs_media_buttons( $editor_id ) {
	...
	<div class="add-files-button">
		<button id="insert-media-button" class="button add-attachment add_media" data-editor="<?php echo esc_attr( $editor_id ); ?>" title="<?php esc_attr_e( 'Dateien hinzufügen', 'bp-docs' ); ?>"><?php echo $img; ?><?php esc_html_e( 'Dateien hinzufügen', 'bp-docs' ); ?></button>
	</div>
	<?php
}
 ...
 *
 * Alle Dokumente content appears in a div.bp-docs. Classes are also included for current theme/parent theme, eg
...
function bp_docs_doc_trash_notice() {
	$status = get_post_status( get_the_ID() );
	if ( 'trash' == $status ) {
		echo ' <span title="' . __( 'Dieses Dokument ist im Papierkorb', 'bp-docs' ) . '" class="bp-docs-trashed-doc-notice">' . __( 'Papierkorb', 'bp-docs' ) . '</span>';
	} elseif ( 'bp_docs_pending' == $status  ) {
		echo ' <span title="' . __( 'Dieser Dokument wartet auf Moderation', 'bp-docs' ) . '" class="bp-docs-pending-doc-notice">' . __( 'Warten auf Moderation', 'bp-docs' ) . '</span>';
	}
}
