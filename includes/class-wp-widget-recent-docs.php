<?php
/**
 * BP Dokumente Recent Docs Widget
...
class BP_Docs_Widget_Recent_Docs extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => __( 'Zeigt die neuesten BP-Dokumente an, die der Besucher lesen kann.' ) );
		parent::__construct( 'widget_recent_bp_docs', _x( '(BP Dokumente) Aktuelle Dokumente', 'widget name', 'bp-docs' ), $widget_ops);
	...
	public function widget( $args, $instance ) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Aktuelle BP-Dokumente', 'bp-docs' );
	...
	public function form( $instance ) {
		...
		?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Titel:', 'bp-docs' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" /></p>

		<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Anzahl anzuzeigender Dokumente:', 'bp-docs' ); ?></label>
		<input class="tiny-text" id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="number" step="1" min="1" value="<?php echo $number; ?>" size="3" /></p>

		<p><input class="checkbox" type="checkbox"<?php checked( $show_date ); ?> id="<?php echo $this->get_field_id( 'show_date' ); ?>" name="<?php echo $this->get_field_name( 'show_date' ); ?>" />
		<label for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php _e( 'Postdatum anzeigen?', 'bp-docs' ); ?></label></p>
		...
		<label for="<?php echo $this->get_field_id( 'context_aware' ); ?>"><?php _e( 'Wenn innerhalb einer Gruppe oder im Profil eines Benutzers verwendet, beschränke die Dokumente auf die aktuelle Gruppe oder die angezeigten Dokumente des Benutzers.', 'bp-docs' ); ?></label></p>
