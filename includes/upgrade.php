...
function bp_docs_upgrade_notice() {
	...
	if ( ! empty( $_GET['bp_docs_upgraded'] ) ) {
		...
		echo '<div class="updated message"><p>' . __( 'Upgrade abgeschlossen!', 'bp-docs' ) . '</p></div>';
	}
	...
	<div class="message error">
		<p><?php _e( 'Vielen Dank für die Aktualisierung von BP Dokumente. Wir müssen einige schnelle Vorgänge ausführen, bevor Deine neuen Dokumente einsatzbereit sind.', 'bp-docs' ) ?></p>
		<p><strong><a href="<?php echo admin_url( 'edit.php?post_type=bp_doc&page=bp-docs-upgrade' ) ?>"><?php _e( 'Klicke hier, um das Upgrade zu starten.', 'bp-docs' ) ?></a></strong></p>
	</div>
	<?php
}
...
function bp_docs_upgrade_menu() {
	add_submenu_page(
		...
		__( 'BP Dokumente Upgrade', 'bp-docs' ),
		__( 'Upgrade', 'bp-docs' ),
		'bp_moderate',
		'bp-docs-upgrade',
...
function bp_docs_upgrade_render() {
	...
	?>
	<div class="wrap">
		<h2><?php _e( 'BP Dokumente Upgrade', 'bp-docs' ) ?></h2>
		...
		<?php if ( 'none' == $status ) : ?>
			...
			<a class="button primary" href="<?php echo $url ?>"><?php _e( 'Starte das Upgrade', 'bp-docs' ) ?></a>
		...
		<?php elseif ( 'complete' == $status ) : ?>
			...
			<p><?php printf( __( 'Migration abgeschlossen! <a href="%s">Dashboard</a>', 'bp-docs' ), admin_url() ) ?></p>
	...
	if ( empty( $udata['group_terms_migrated'] ) ) {
		...
		if ( $group_parent_term ) {
			...
			foreach ( $group_terms as $group_term ) {
				...
				$new_desc = sprintf( __( 'Mit der Gruppe %s verknüpfte Dokumente', 'bp-docs' ), $group_term->description );
		...
		$udata['message'] = __( 'Gruppenbegriffe migriert. Migriere jetzt die Dokumenten-Zugriffsbedingungen....', 'bp-docs' );
		...
		while ( $counter < 5 ) {
			...
			$udata['message'] = sprintf( __( '%s von %s Dokumenten migriert. Migration....', 'bp-docs' ), $udata['done'], $udata['total'] );
			$udata['refresh_url'] = add_query_arg( array(
				'do_upgrade' => '1',
				'_wpnonce'   => wp_create_nonce( 'bp-docs-upgrade' ),
			), $url_base );
