...
class BP_Docs_Query {
	...
	function save( $passed_args = array() ) {
		...
		$result = array(
			'error'    => false,
			'message'  => __( 'Unbekannter Fehler. Bitte versuche es erneut.', 'bp-docs' ),
			'redirect' => 'create'
		);
		...
		if ( true === $result['error'] ) {
			...
			$result['message'] = __( 'Das Titelfeld ist erforderlich.', 'bp-docs' );
			...
			if ( ! $this->doc_id ) {
				...
				if ( $this->is_new_doc  ) {
					$result['message'] = __( 'Beim Erstellen des Dokuments ist ein Fehler aufgetreten.', 'bp-docs' );
					$result['redirect'] = 'create';
				} else {
					$result['message'] = __( 'Beim Speichern des Dokuments ist ein Fehler aufgetreten.', 'bp-docs' );
				...
				if ( $this->is_new_doc ) {
					// New doc saved.
					$result['message'] = __( 'Dokument erfolgreich erstellt!', 'bp-docs' );
				} elseif ( $args['is_auto'] ) {
					// Doc update was an autosave.
					$result['message'] = __( 'Du hast im Bearbeitungsmodus etwas zu lange im Leerlauf gearbeitet. Damit andere das Dokument bearbeiten können, an dem Du gearbeitet hast, wurden Deine Änderungen automatisch gespeichert. Klicke auf die Schaltfläche Bearbeiten, um zum Bearbeitungsmodus zurückzukehren.', 'bp-docs' );
				} else {
					// Existing Doc updated.
					$result['message'] = __( 'Dokument erfolgreich bearbeitet!', 'bp-docs' );
				}
