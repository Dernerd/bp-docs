...
class BP_Docs_Moderation {
	...
	public function register_docs_pending_status() {
		$args = array(
			'label'                     => _x( 'Warten auf Moderation', 'Allgemeiner Name des Status des ausstehenden Dokuments', 'bp-docs' ),
			'label_count'               => _n_noop( 'Warten auf Moderation (%s)',  'Warten auf Moderation (%s)', 'bp-docs' ),
	...
	function add_moderated_label( $post_states, $post ) {
		if ( $this->pending_status == $post->post_status ) {
			$post_states[] = __( 'Warten auf Moderation', 'bp-docs' );
		}
		return $post_states;
	}
}
