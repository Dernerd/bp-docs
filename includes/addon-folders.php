...
class BP_Docs_Folders {
	...
	public function register_post_type() {
		$labels = array(
			'name' => __( 'BP Dokumenten-Ordner', 'bp-docs' ),
			'singular_name' => __( 'BP Dokumenten-Ordner', 'bp-docs' ),
			'menu_name' => __( 'Dokumentenordner', 'bp-docs' ),
			'name_admin_bar' => __( 'Dokumenten-Ordner', 'bp-docs' ),
		);
	...
	public function enqueue_assets() {
		wp_register_script( 'bp-docs-chosen', plugins_url() . '/bp-docs/lib/js/chosen/chosen.jquery.min.js', array( 'jquery' ) );
		...
		wp_enqueue_script( 'bp-docs-folders', plugins_url() . '/bp-docs/includes/js/folders.js', $js_requirements );

		wp_register_style( 'bp-docs-chosen', plugins_url() . '/bp-docs/lib/css/chosen/chosen.min.css' );
		if ( is_rtl() ) {
			wp_enqueue_style( 'bp-docs-folders', plugins_url() . '/bp-docs/includes/css-rtl/folders.css', array( 'bp-docs-chosen' ) );
		} else {
			wp_enqueue_style( 'bp-docs-folders', plugins_url() . '/bp-docs/includes/css/folders.css', array( 'bp-docs-chosen' ) );
		}
		...
		wp_localize_script( 'bp-docs-folders', 'BP_Docs_Folders', array(
			'folders_tab_label' => _x( 'Ordner', 'Doc edit tab name', 'bp-docs' ),
			'folders_tab_label_groups' => _x( 'Gruppenordner', 'Doc edit tab name', 'bp-docs' ),
	...
	switch ( $_POST['existing-or-new-folder'] ) {
		...
		case 'new' :
			...
			if ( empty( $folder_name ) ) {
				bp_core_add_message( __( 'Du musst einen Ordnernamen angeben.', 'bp-docs' ), 'error' );
			} else {
				$folder_args = array(
					'name' => $folder_name,
				);
...
function bp_docs_process_folder_edit_cb() {
	...
	if ( ! wp_verify_nonce( $nonce, 'bp-docs-edit-folder-' . $folder_id ) ) {
		bp_core_add_message( __( 'Beim Bearbeiten dieses Ordners ist ein Problem aufgetreten. Bitte versuche es erneut.', 'bp-docs' ), 'error' );
	...
	if ( ! empty( $success ) && ! is_wp_error( $success ) ) {
		bp_core_add_message( __( 'Ordner erfolgreich aktualisiert.', 'bp-docs' ), 'success' );
	} else {
		bp_core_add_message( __( 'Es gab ein Problem beim Bearbeiten dieses Ordners. Bitte versuche es erneut.', 'bp-docs' ), 'error' );
	}
...
function bp_docs_process_folder_create_cb() {
	...
	if ( ! empty( $success ) && ! is_wp_error( $success ) ) {
		bp_core_add_message( __( 'Ordner erfolgreich erstellt.', 'bp-docs' ), 'success' );
	} else {
		bp_core_add_message( __( 'Beim Erstellen des Ordners ist ein Problem aufgetreten. Bitte versuche es erneut.', 'bp-docs' ), 'error' );
	}
...
function bp_docs_process_folder_delete_cb() {
	...
	if ( $deleted ) {
		bp_core_add_message( __( 'Ordner gelöscht.', 'bp-docs' ) );
	} else {
		bp_core_add_message( __( 'Ordner konnte nicht gelöscht werden.', 'bp-docs' ) );
	}
...
function bp_docs_folder_selector( $args = array() ) {
	...
	if ( ! empty( $r['user_id'] ) ) {
		...
		if ( ! empty( $user->ID ) ) {
			$label = $r['user_id'] === bp_loggedin_user_id() ? __( 'Meine Ordner', 'bp-docs' ) : bp_core_get_user_displayname( $r['user_id'] );
			$types['user'] = array(
				'label' => $label,
				'folders' => bp_docs_get_folders( array(
					'display'   => 'flat',
	...
	$options = '<option value="">' . __( ' - Keinen - ', 'bp-docs' ) . '</option>' . $options;
...
function bp_docs_folder_type_selector( $args = array() ) {
	...
	$type_selector .=   '<option ' . selected( $r['selected'], 'me', false ) . ' value="me">' . __( 'Persönlich', 'bp-docs' ) . '</option>';

	if ( isset( $group_selector ) ) {
		$type_selector .=   '<optgroup label="' . __( 'Gruppenspezifisch', 'bp-docs' ) . '">';
...
function bp_docs_create_new_folder_markup( $args = array() ) {
	...
	<div class="folder-type-selector-div <?php echo $folder_type_class ?>">
		<label for="new-folder-type"><?php _e( 'Ordnertyp', 'bp-docs' ) ?></label>
	...
	<div style="clear:both"></div>
	<label for="new-folder-parent"><?php _e( 'Eltern (optional)', 'bp-docs' ) ?></label>
	...
	<div id="doc-folders" class="doc-meta-box">
		<div class="toggleable <?php bp_docs_toggleable_open_or_closed_class( 'folders-meta-box' ) ?>">
			<p id="folders-toggle-edit" class="toggle-switch">
				<span class="hide-if-js toggle-link-no-js"><?php _e( 'Ordner', 'bp-docs' ) ?></span>
				<a class="hide-if-no-js toggle-link" id="folders-toggle-link" href="#"><span class="show-pane plus-or-minus"></span><span class="toggle-title"><?php _e( 'Ordner', 'bp-docs' ) ?></span></a>
			</p>
				...
				<table class="toggle-table" id="toggle-table-folders">
					<tr>
						<td class="desc-column">
							<label for="bp_docs_tag"><?php _e( 'Wähle Ordner für dieses Dokument.', 'bp-docs' ) ?></label>
						</td>
						...
						<td>
							<div class="existing-or-new-selector">
								...
								<label for="use-existing-folder" class="radio-label"><?php _e( 'Verwende vorhandenen Ordner', 'bp-docs' ) ?></label><br />
							...
							<div class="existing-or-new-selector" id="new-folder-block">
								...
								<label for="create-new-folder" class="radio-label"><?php _e( 'Neuen Ordner erstellen', 'bp-docs' ) ?></label>
								<div class="selector-content">
...
function bp_docs_folder_info_header_message( $message, $filters ) {
	if ( ! empty( $filters['folders'] ) ) {
		...
		$message[] = sprintf(
			_n( 'Du zeigst Dokumente in folgendem Ordner an: %s','Du zeigst Dokumente in folgenden Ordnern an: %s', count( $folders ), 'bp-docs' ),
...
class BP_Docs_Folder_Dropdown_Walker extends Walker {
	...
	function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		...
		if ( '' === $title ) {
			$title = sprintf( __( '#%d (kein Titel)', 'bp-docs' ), $page->ID );
		}
	...
	public function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		...
		if ( empty( $page->post_parent ) ) {
			...
			$type_selector_markup = sprintf(
				...
				__( 'Typ', 'bp-docs' ), // type label text
...
<li class="folder folder-edit-closed" data-folder-id="%d">
	<div class="folder-info">
		...
		<div class="folder-details">
			...
			__( 'Bearbeiten', 'bp-docs' ), // folder-edit-toggle text
			__( 'Schließen', 'bp-docs' ), // folder-edit-close text
			...
			__( 'Änderungen speichern', 'bp-docs' ), // save button text
			$delete_link_markup
		);
	}
...
class BP_Docs_Folder_Walker extends Walker {
	...
	public function end_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		...
		$output .= '<li class="folder-empty">' . __( 'Dieser Ordner enthält keine Dokumente.', 'bp-docs' ) . '</li>';
