...
//		add_action( 'wp', array( $this, 'setup_attachments' ), 1 );
	...
	function setup_globals( $args = array() ) {
		...
		$globals = array(
			...
			'search_string'         => __( 'Dokumente suchen...', 'bp-docs' ),
		);
	...
	public function setup_admin_bar( $wp_admin_nav = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			$wp_admin_nav[] = array(
				...
				'title'  => __( 'Von mir begonnen', 'bp-docs' ),
			...
			$wp_admin_nav[] = array(
				...
				'title'  => __( 'Von mir bearbeitet', 'bp-docs' ),
			...
			$wp_admin_nav[] = array(
				...
				'title'  => __( 'Neues Dokument erstellen', 'bp-docs' ),
	...
	function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		$main_nav = array(
			...
			// Disabled count for now. See https://github.com/boonebgorges/bp-docs/issues/261
		...
		$mydocs_label = bp_is_my_profile() ? __( 'Meine Dokumente ', 'bp-docs' ) : sprintf( __( '%s&#8217;s Dokumente', 'bp-docs' ), bp_get_user_firstname( bp_get_displayed_user_fullname() ) );

		$sub_nav[] = array(
			'name'            => bp_is_my_profile() ? __( 'Von mir begonnen', 'bp-docs' ) : sprintf( __( 'Begonnen von %s', 'bp-docs' ), bp_get_user_firstname() ),
		...
		$sub_nav[] = array(
			'name'            => bp_is_my_profile() ? __( 'Von mir bearbeitet', 'bp-docs' ) : sprintf( __( 'Bearbeitet von %s', 'bp-docs' ), bp_get_user_firstname() ),
	...
	function catch_page_load() {
		...
		if ( bp_docs_is_doc_edit() ) {
			if ( current_user_can( 'bp_docs_edit' ) ) {
				...
				if ( $doc ) {
					...
					if ( $lock ) {
						bp_core_add_message( sprintf( __( 'Dieses Dokument wird derzeit von %s bearbeitet. Um Überschreibungen zu vermeiden, kannst Du diese erst bearbeiten, wenn dieser Benutzer fertig ist. Bitte versuche es in einigen Minuten erneut.', 'bp-docs' ), bp_core_get_user_displayname( $lock ) ), 'error' );
				...
				bp_core_add_message( __( 'Du hast keine Berechtigung zum Bearbeiten des Dokuments.', 'bp-docs' ), 'error' );
		...
		if ( bp_docs_is_doc_create() ) {
			if ( ! current_user_can( 'bp_docs_create' ) ) {
				...
				bp_core_add_message( __( 'Du hast keine Berechtigung zum Erstellen eines Dokuments in dieser Gruppe.', 'bp-docs' ), 'error' );
		...
		if ( !empty( $bp->bp_docs->current_view ) && 'history' == $bp->bp_docs->current_view ) {
			if ( ! current_user_can( 'bp_docs_view_history' ) ) {
				...
				bp_core_add_message( __( 'Du hast keine Berechtigung, den Verlauf dieses Dokuments anzuzeigen.', 'bp-docs' ), 'error' );
		...
		if ( !empty( $_GET['bpd_action'] ) && $_GET['bpd_action'] == 'cancel_edit_lock' ) {
			...
			if ( is_super_admin() || bp_group_is_admin() ) {
				...
				bp_core_add_message( __( 'Sperre erfolgreich entfernt', 'bp-docs' ) );
				bp_core_redirect( bp_docs_get_doc_link( $doc->ID ) );
				die();
			}
		}
		...
		if ( bp_docs_is_doc_read() && ! empty( $_GET['delete'] ) ) {
			...
			if ( current_user_can( 'bp_docs_manage' ) ) {
				...
				if ( bp_docs_trash_doc( $delete_doc_id, $force_delete ) ) {
					bp_core_add_message( __( 'Dokument erfolgreich gelöscht!', 'bp-docs' ) );
				} else {
					bp_core_add_message( __( 'Dokument konnte nicht gelöscht werden.', 'bp-docs' ) );
				}
			} else {
				bp_core_add_message( __( 'Du hast keine Berechtigung zum Löschen dieses Dokuments.', 'bp-docs' ), 'error' );
			}
		...
		if ( bp_docs_is_doc_read() && ! empty( $_GET['untrash'] ) && ! empty( $_GET['doc_id'] ) ) {
			...
			if ( current_user_can( 'bp_docs_manage', $untrash_doc_id ) ) {
				if ( bp_docs_untrash_doc( $untrash_doc_id ) ) {
					bp_core_add_message( __( 'Dokument erfolgreich aus dem Papierkorb entfernt!', 'bp-docs' ) );
				} else {
					bp_core_add_message( __( 'Dokument konnte nicht aus dem Papierkorb entfernt werden.', 'bp-docs' ) );
				}
			} else {
				bp_core_add_message( __( 'Du hast keine Berechtigung, dieses Dokument aus dem Papierkorb zu entfernen.', 'bp-docs' ), 'error' );
			}
		...
		if ( bp_docs_is_doc_read() && ! empty( $_GET[ BP_DOCS_UNLINK_FROM_GROUP_SLUG ] ) && ! empty( $_GET['doc_id'] ) && ! empty( $_GET['group_id'] ) ) {
			...
			if ( current_user_can( 'bp_docs_dissociate_from_group', $unlink_group_id ) ) {
				if ( bp_docs_unlink_from_group( $unlink_doc_id, $unlink_group_id ) ) {
					bp_core_add_message( __( 'Dokument erfolgreich aus der Gruppe entfernt', 'bp-docs' ) );
				} else {
					bp_core_add_message( __( 'Dokument konnte nicht aus der Gruppe entfernt werden.', 'bp-docs' ) );
				}
			} else {
				bp_core_add_message( __( 'Du hast keine Berechtigung, dieses Dokument aus dieser Gruppe zu entfernen.', 'bp-docs' ), 'error' );
			}
	 ...
	 * functions.
	 *
	 * BP Dokumente has a completely flat architecture for URLs, where
	...
	public static function filter_markup() {
		...
		<div id="docs-filter-section-search" class="docs-filter-section<?php if ( $has_search ) : ?> docs-filter-section-open<?php endif ?>">
			<form action="<?php echo esc_url( $form_action ); ?>" method="get">
				<label for="docs-search" class="screen-reader-text"><?php echo esc_html_e( 'Suche', 'bp-docs' ); ?></label>
				<input id="docs-search" name="s" value="<?php the_search_query() ?>">
				<input name="search_submit" type="submit" value="<?php _e( 'Suche', 'bp-docs' ) ?>" />
	...
	function enqueue_scripts() {
		...
		if ( in_array( bp_docs_get_docs_slug(), $this->slugstocheck ) || bp_docs_is_single_doc() || bp_docs_is_global_directory() || bp_docs_is_mygroups_directory() || bp_docs_is_doc_create() ) {
			...
			$strings = array(
				'upload_title' => __( 'Datei hochladen', 'bp-docs' ),
				'upload_button' => __( 'OK', 'bp-docs' ),
				'still_working'	=> __( 'Immer noch am arbeiten?', 'bp-docs' ),
				'and_x_more' => __( 'und %d mehr', 'bp-docs' ),
