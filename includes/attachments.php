...
class BP_Docs_Attachments {
	...
	function catch_attachment_request() {
		if ( ! empty( $_GET['bp-attachment'] ) ) {
			...
			if ( ! $this->filename_is_safe( $fn ) ) {
				wp_die( __( 'Datei nicht gefunden.', 'bp-docs' ) );
			}
	...
	public function create_htaccess() {
		...
		if ( ! empty( $rules ) ) {
			...
			insert_with_markers( $htaccess_path, 'BP Dokumente', $rules );
		}
	}
...
//		}
	...
	public function catch_delete_request() {
		...
		if ( wp_delete_attachment( $attachment_id ) ) {
			bp_core_add_message( __( 'Anhang gelöscht', 'bp-docs' ) );
		} else {
			bp_core_add_message( __( 'Anhang konnte nicht gelöscht werden', 'bp-docs' ), 'error' );
		}
	...
	public static function filter_markup() {
		...
		<div id="docs-filter-section-attachments" class="docs-filter-section<?php if ( $has_attachment ) : ?> docs-filter-section-open<?php endif ?>">
			<form method="get" action="<?php echo $form_action ?>">
				<label for="has-attachment"><?php _e( 'Hat Anhang?', 'bp-docs' ) ?></label>
				<select id="has-attachment" name="has-attachment">
					<option value="yes"<?php selected( $has_attachment, 'yes' ) ?>><?php _e( 'Ja', 'bp-docs' ) ?></option>
					<option value="no"<?php selected( $has_attachment, 'no' ) ?>><?php _e( 'Nein', 'bp-docs' ) ?></option>
					<option value=""<?php selected( $has_attachment, '' ) ?>><?php _e( 'Ist egal', 'bp-docs' ) ?></option>
				</select>
	...
	public function admin_notice() {
		...
		if ( $is_nginx ) {
			$help_url = 'https://github.com/boonebgorges/bp-docs/wiki/Attachment-Privacy#nginx';

			$help_p  = __( 'Es sieht so aus, als würdest Du <strong>nginx</strong> ausführen. Wir empfehlen die folgende Einstellung in Deiner Seiten-Konfigurationsdatei:', 'bp-docs' );
    ...
    rewrite ^.*' . str_replace( '/wp-content/', '', $att_url ) . '([0-9]+)/(.*) /?p=$1&bp-attachment=$2 permanent;
		...
		if ( $is_iis7 ) {
			$help_url = 'https://github.com/boonebgorges/bp-docs/wiki/Attachment-Privacy#iis7';

			$help_p  = __( 'Es sieht so aus, als würdest Du <strong>IIS 7</strong> ausführen. Wir empfehlen die folgende Einstellung in Deiner Web.config-Datei:', 'bp-docs' );
			$help_p .= '<pre><code>&lt;rule name="bp-docs-attachments">
...
&lt;/rule> </code></pre>';
		...
		if ( $is_apache ) {
			$help_url = 'https://github.com/boonebgorges/bp-docs/wiki/Attachment-Privacy#apache';
			$help_p = __( 'Es sieht so aus, als würdest Du <strong>Apache</strong> ausführen. Die wahrscheinlichste Ursache für Dein Problem ist, dass die Anweisung <code>AllowOverride</code> entweder global (<code>httpd.conf</code>) oder in einer <code>VirtualHost</code>-Definition deaktiviert wurde. Wende Dich an Deinen Hosting-Anbieter, um Unterstützung zu erhalten.', 'bp-docs' );
		}

		?>

		<div class="message error">
			<p><?php _e( '<strong>Dein BP collporate Docs-Anhangsverzeichnis ist öffentlich zugänglich.</strong> Dokumentanhänge sind nicht ordnungsgemäß vor direkter Anzeige geschützt, selbst wenn die übergeordneten Dokumente nicht öffentlich sind.', 'bp-docs' ) ?></p>
			...
			<?php if ( $help_url ) : ?>
				<p><?php printf( __( 'Weitere Informationen findest Du unter <a href="%s">dieser Wiki-Seite</a>.', 'bp-docs' ), $help_url ) ?></p>
			<?php endif ?>

			<p><a href="<?php echo $dismiss_url ?>"><?php _e( 'Diese Nachricht verwerfen', 'bp-docs' ) ?></a></p>
		</div>
		<?php
	}
	...
	public function check_is_protected( $force_check = true ) {
		...
		$test_text = 'Dies ist ein Test der Funktion für geschützte Anhänge von BP collporate Docs. Bitte nicht entfernen.';
		...
		if ( ! file_exists( $test_file_dir ) ) {
			...
			if ( $is_apache ) {
				...
				if ( ! empty( $rules ) ) {
					...
					insert_with_markers( $test_dir . DIRECTORY_SEPARATOR . '.htaccess', 'BP Dokumente', $rules );
				}
			}
