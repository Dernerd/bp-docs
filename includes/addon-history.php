...
/**
 * The History tab add-on for BP Dokumente
 *
...
class BP_Docs_History {
	...
	function setup_action() {
		...
		case 'restore' :
			...
			bp_core_add_message( sprintf( __( 'Du hast das Dokument erfolgreich auf die Revision von %s zurückgesetzt.', 'bp-docs' ), $this->revision->post_date ) );
		...
		case 'diff' :
			...
			$h2 = sprintf( __( 'Vergleiche Revisionen von &#8220;%1$s&#8221;', 'bp-docs' ), $post_title );
			$title = __( 'Überarbeitungen', 'bp-docs' );
		...
		default :
			...
			$h2 = sprintf( __( 'Revision für &#8220;%1$s&#8221; erstellt am %2$s', 'bp-docs' ), $post_title, $revision_title );
			$title = __( 'Überarbeitungen', 'bp-docs' );
	...
	foreach ( $revisions as $revision ) {
		...
		if ( 'form-table' == $format ) {
			...
			if ( $post->ID != $revision->ID && $can_edit_post )
				$actions = '<a class="confirm" href="' . wp_nonce_url( add_query_arg( array( 'revision' => $revision->ID, 'action' => 'restore' ), $base_url ), "restore-post_$post->ID|$revision->ID" ) . '">' . __( 'Wiederherstellen', 'bp-docs' ) . '</a>';
			else
				$actions = '';

			$rows .= "<tr$class>\n";
			$rows .= "\t<th style='white-space:nowrap;text-align:center' scope='row'><input type='radio' name='left' value='$revision->ID'$left_checked id='left-$revision->ID' /><label class='screen-reader-text' for='left-$revision->ID'>" . __( 'Alt', 'bp-docs' ) . "</label></th>\n";
			$rows .= "\t<th style='white-space:nowrap;text-align:center' scope='row'><input type='radio' name='right' value='$revision->ID'$right_checked id='right-$revision->ID' /><label class='screen-reader-text' for='right-$revision->ID'>" . __( 'Neu', 'bp-docs' ) . "</label></th>\n";
			$rows .= "\t<td>$date</td>\n";
			$rows .= "\t<td>$name</td>\n";
...
<div class="tablenav">
	<div class="alignleft">
		<input type="submit" class="button-secondary" value="<?php esc_attr_e( 'Revisionen vergleichen', 'bp-docs' ); ?>" />
...
<thead>
<tr>
	<th scope="col"><?php /* translators: column name in revisons */ _e( 'Alt', 'bp-docs' ); ?></th>
	<th scope="col"><?php /* translators: column name in revisons */ _e( 'Neu', 'bp-docs' ); ?></th>
	<th scope="col"><?php /* translators: column name in revisons */ _e( 'Datum erstellt', 'bp-docs' ); ?></th>
...
function bp_docs_history_tab() {
	if ( current_user_can( 'bp_docs_view_history' ) ) : ?>
		<li<?php if ( bp_docs_is_doc_history() ) : ?> class="current"<?php endif ?>>
			<a href="<?php echo bp_docs_get_doc_link() . BP_DOCS_HISTORY_SLUG ?>"><?php _e( 'Verlauf', 'bp-docs' ) ?></a>
		</li>
	<?php endif;
}
