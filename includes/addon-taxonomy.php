...
class BP_Docs_Taxonomy {
	...
	function info_header_message( $message, $filters ) {
		...
		if ( ! empty( $filters['tags'] ) ) {
			...
			$message[] = sprintf( __( 'Du zeigst Dokumente mit den folgenden Tags an: %s', 'bp-docs' ), implode( ', ', $tagtext ) );
		}
	...
	function filter_markup() {
		...
		<div id="docs-filter-section-tags" class="docs-filter-section<?php if ( $tag_filter ) : ?> docs-filter-section-open<?php endif ?>">
			...
			<?php if ( ! empty( $existing_terms ) ) : ?>
				...
				<?php endforeach ?>
			<?php else: ?>
				<li><?php _e( 'Keine Tags zum Anzeigen.', 'bp-docs' )  ?></li>
			<?php endif; ?>
			</ul>
		</div>
...
function bp_docs_get_tag_link( $args = array() ) {
	...
	$html = '<a href="' . $url . '" title="' . sprintf( __( 'Dokumente markiert %s', 'bp-docs' ), esc_attr( $tag ) ) . '">' . esc_html( $tag ) . '</a>';
...
function bp_docs_post_categories_meta_box( $post ) {
	...
	<div id="taxonomy-<?php echo $taxonomy; ?>" class="categorydiv">
		<ul id="<?php echo $taxonomy; ?>-tabs" class="category-tabs">
			...
			<li class="hide-if-no-js"><a href="#<?php echo $taxonomy; ?>-pop" tabindex="3"><?php _e( 'Am meisten benutzt', 'bp-docs' ); ?></a></li>
		</ul>
