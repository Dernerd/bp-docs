...
class BP_Docs_Groups_Integration {
	...
	public function pre_save_check_group_association( $result, $args ) {
		...
		if ( ! empty( $args['group_id'] ) && ! user_can( $args['author_id'], 'bp_docs_associate_with_group', $args['group_id'] ) ) {
			$result['error']    = true;
			$result['message']	= __( 'Du darfst dieser Gruppe kein Dokument zuordnen.', 'bp-docs' );
	...
	public static function get_access_options( $options, $settings_field, $doc_id = 0, $group_id = 0 ) {
		...
		if ( $group_id && current_user_can( 'bp_docs_associate_with_group', $group_id ) ) {
			...
			$options[40] = array(
				'name'  => 'group-members',
				'label' => sprintf( __( 'Mitglieder von %s', 'bp-docs' ), $group->name )
			);
			...
			if ( $is_admin || $is_mod ) {
				$options[50] = array(
					'name'  => 'admins-mods',
					'label' => sprintf( __( 'Admins und Mods von %s', 'bp-docs' ), $group->name )
				);
			}
	...
	function activity_action( $action, $user_link, $doc_link, $is_new_doc, $query ) {
		...
		if ( $group_id ) {
			...
			if ( $is_new_doc ) {
				$action = sprintf( __( '%1$s hat das Dokument %2$s in der Gruppe %3$s erstellt', 'bp-docs' ), $user_link, $doc_link, $group_link );
			} else {
				$action = sprintf( __( '%1$s hat das Dokument %2$s in der Gruppe %3$s bearbeitet', 'bp-docs' ), $user_link, $doc_link, $group_link );
			}
		}
	...
	function comment_activity_action( $action, $user_link, $comment_link, $component, $item ) {
		if ( 'groups' == $component ) {
			...
			$action 	= sprintf( __( '%1$s hat das Dokument %2$s in der Gruppe %3$s kommentiert', 'bp-docs' ), $user_link, $comment_link, $group_link );
		}
	...
	function groups_th() {
		...
		<th scope="column" class="groups-cell"><?php _e( 'Gruppe', 'bp-docs' ); ?></th>
	...
	function add_doc_action_unlink_from_group_link( $links, $doc_id ) {
		...
		if ( $group_id = bp_get_current_group_id() ) {
			if ( current_user_can( 'bp_docs_dissociate_from_group', $group_id ) ) {
				$links[] = '<a href="' . bp_docs_get_unlink_from_group_link( $doc_id, $group_id ) . '" class="unlink-from-group confirm">' . __( 'Verbindung zur Gruppe trennen', 'bp-docs' ) . '</a>';
			}
		}
		return $links;
	}
			...
			if ( empty( $doc->post_title ) ) {
				...
				if ( isset( $bp->action_variables[1] ) ) {
					if ( BP_DOCS_EDIT_SLUG == $bp->action_variables[1] ) {
						$title[] = __( 'Bearbeiten', 'bp-docs' );
					} else if ( BP_DOCS_HISTORY_SLUG == $bp->action_variables[1] ) {
						$title[] = __( 'Verlauf', 'bp-docs' );
					}
				}
			}
...
class BP_Docs_Group_Extension extends BP_Group_Extension {
	...
	public function __construct() {
		...
		$this->name 			= !empty( $bp_docs_tab_name ) ? $bp_docs_tab_name : __( 'Dokumente', 'bp-docs' );
	...
	function edit_screen( $group_id = null ) {
		...
		?>
		<p>
			<input type="submit" value="<?php _e( 'Änderungen speichern', 'bp-docs' ) ?>" id="save" name="save" />
		</p>
		<?php
	...
	function edit_screen_save( $group_id = null ) {
		...
		if ( !$success )
			bp_core_add_message( __( 'Fehler beim speichern. Bitte versuche es erneut', 'bp-docs' ), 'error' );
		else
			bp_core_add_message( __( 'Einstellungen erfolgreich gespeichert', 'bp-docs' ) );
	...
	function admin_markup() {
		...
		<h2><?php _e( 'Dokumente', 'bp-docs' ) ?></h2>

		<p><?php _e( 'Dokumente ist ein leistungsstarkes Tool für die Zusammenarbeit mit Mitgliedern Deiner Gruppe. Dokumente ist eine Mischung aus Dokumenteditor und Wiki und ermöglicht es Dir, Dokumente gemeinsam mit Deinen Gruppenmitgliedern zu verfassen und zu bearbeiten. Diese kannst Du dann so sortieren und markieren, dass Deine Gruppe ihre Arbeit erledigen kann.', 'bp-docs' ) ?></p>

		<p>
			 <label for="bp-docs-group-enable"> <input type="checkbox" name="bp-docs[group-enable]" id="bp-docs-group-enable" value="1" <?php checked( $group_enable, true ) ?> /> <?php _e( 'Aktiviere Dokumente für diese Gruppe', 'bp-docs' ) ?></label>
		</p>

		<div id="group-doc-options" <?php if ( !$group_enable ) : ?>class="hidden"<?php endif ?>>
			<h3><?php _e( 'Optionen', 'bp-docs' ) ?></h3>

			<label for="bp-docs-can-create"><?php _e( 'Mindestrolle für die Zuordnung von Dokumenten zu dieser Gruppe:', 'bp-docs' ) ?></label>
			<select name="bp-docs[can-create]" id="bp-docs-can-create">
				<option value="admin" <?php selected( $can_create, 'admin' ) ?>><?php _e( 'Gruppenadministrator', 'bp-docs' ) ?></option>
				<option value="mod" <?php selected( $can_create, 'mod' ) ?>><?php _e( 'Gruppenmoderator', 'bp-docs' ) ?></option>
				<option value="member" <?php selected( $can_create, 'member' ) ?>><?php _e( 'Gruppenmitglied', 'bp-docs' ) ?></option>
			</select>
		</div>
...
?>
	<li<?php if ( $bp->bp_docs->current_view == 'list' ) : ?> class="current"<?php endif; ?>><a href="<?php echo $bp->root_domain . '/' . $groups_slug ?>/<?php echo $group->slug ?>/<?php echo $bp->bp_docs->slug ?>/"><?php _e( 'Dokumente anzeigen', 'bp-docs' ) ?></a></li>

	<?php if ( current_user_can( 'bp_docs_create' ) ) : ?>
		<li<?php if ( 'create' == $bp->bp_docs->current_view ) : ?> class="current"<?php endif; ?>><a href="<?php echo $bp->root_domain . '/' . $groups_slug ?>/<?php echo $group->slug ?>/<?php echo $bp->bp_docs->slug ?>/create"><?php _e( 'Neues Dokument', 'bp-docs' ) ?></a></li>
	<?php endif ?>
	...
	if ( bp_is_group() ) {
		$group_crumbs = array(
			sprintf(
				...
				sprintf( _x( '%s&#8217;s Dokumente', 'group Docs directory breadcrumb', 'bp-docs' ), esc_html( bp_get_current_group_name() ) )
			),
		);
	...
	if ( $user_has_access ) {
		$group_crumbs = array(
			sprintf(
				...
				sprintf( esc_html__( '%s&#8217;s Dokumente', 'bp-docs' ), esc_html( $group->name ) )
			),
		);
