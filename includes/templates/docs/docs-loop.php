		...
		<div class="folder-action-links">
			<?php if ( current_user_can( 'bp_docs_manage_folders' ) ) : ?>
				<div class="manage-folders-link">
					<a href="<?php bp_docs_manage_folders_url() ?>"><?php _e( 'Ordner verwalten', 'bp-docs' ) ?></a>
				</div>
			<?php endif ?>

			<div class="toggle-folders-link hide-if-no-js">
				<a href="#" class="toggle-folders" id="toggle-folders-hide"><?php _e( 'Ordner ausblenden', 'bp-docs' ) ?></a>
				<a href="#" class="toggle-folders" id="toggle-folders-show"><?php _e( 'Ordner anzeigen', 'bp-docs' ) ?></a>
			</div>
		</div>
	<?php endif; ?>
		...
		<?php if ( ! empty( $_GET['folder'] ) ) : ?>
			<tr class="folder-row">
				...
				<td class="folder-row-name" colspan=10>
					<a href="<?php echo esc_url( bp_docs_get_parent_folder_url() ) ?>" class="up-one-folder"><?php bp_docs_genericon( 'category', 0 ); ?><span class="screen-reader-text"><?php _e( 'Gehe einen Ordner hoch', 'bp-docs' ) ?></span><?php _ex( '..', 'einen Ordner hoch', 'bp-docs' ) ?></a>
				</td>
			</tr>
		<?php endif ?>
		...
		<tr class="no-docs-row<?php if ( $has_docs || $has_folders ) { echo ' hide'; } ?>">
			...
			<td class="title-cell">
				<?php if ( bp_docs_current_user_can_create_in_context() ) : ?>
					<p class="no-docs"><?php printf( __( 'Für diese Ansicht sind keine Dokumente vorhanden. Erstelle ein <a href="%s">Dokument</a>?', 'bp-docs' ), bp_docs_get_create_link() ); ?>
				<?php else : ?>
					<p class="no-docs"><?php _e( 'Für diese Ansicht sind keine Dokumente vorhanden.', 'bp-docs' ); ?></p>
				<?php endif; ?>
			</td>
	...
	<?php if ( $has_docs ) : ?>
		<div id="bp-docs-pagination">
			<div id="bp-docs-pagination-count">
				<?php printf( __( 'Zeige %1$s-%2$s von %3$s Dokumente', 'bp-docs' ), bp_docs_get_current_docs_start(), bp_docs_get_current_docs_end(), bp_docs_get_total_docs_num() ); ?>
			</div>
