	...
	<?php endwhile ?>
		...
		<tr class="folder-meta-info">
			...
			<td class="folder-meta-info-statement" colspan=10>
				<?php printf( __( 'Anzeigen von %1$s-%2$s von %3$s-Dokumenten in diesem Ordner.', 'bp-docs' ), bp_docs_get_current_docs_start(), bp_docs_get_current_docs_end(), bp_docs_get_total_docs_num() ) ?> <br/>
				<a href="<?php echo esc_url( bp_docs_get_folder_url( $current_folder_id ) ); ?>"><?php printf( __( 'Alle Dokumente in <strong>%s</strong> anzeigen.', 'bp-docs' ), get_the_title( $current_folder_id ) ); ?></a>
			</td>
		</tr>
<?php endif; ?>
	...
	<tr class="no-docs-row<?php if ( $has_docs || $has_folders ) { echo ' hide'; } ?>">
		...
		<td class="title-cell">
			<?php if ( bp_docs_current_user_can_create_in_context() ) : ?>
				<p class="no-docs"><?php printf( __( 'Für diese Ansicht sind keine Dokumente vorhanden. Warum nicht <a href="%s">eines erstellen</a>?', 'bp-docs' ), bp_docs_get_create_link() ); ?>
			<?php else : ?>
