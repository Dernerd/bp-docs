...
class BP_Docs {
	 ...
	 *    - WP_LANG_DIR/plugins/bp-docs-LOCALE.mo
	 *    - WP_PLUGIN_DIR/bp-docs/languages/bp-docs-LOCALE.mo
	 *
	 * @since 1.0.2
	 */
	function load_plugin_textdomain() {
		 ...
		 * wp-content/languages/plugins/bp-docs-es_ES.mo
		 ...
		 * wp-content/bp-docs/languages/bp-docs-es_ES.mo
		 */
		load_plugin_textdomain( 'bp-docs', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' );
	}

	/**
	 * Includes files needed by BP Dokumente
	 *
	 * @since 1.0-beta
	 */
	function includes() {
	 ...
	 * BP Dokumente, as well as other dependent plugins, to hook into the loading process in
	 * an orderly fashion.
	 *
	 * @since 1.2
	 */
	...
	function load_constants() {
		if ( ! defined( 'BP_DOCS_PLUGIN_SLUG' ) ) {
			define( 'BP_DOCS_PLUGIN_SLUG', 'bp-docs' );
		}
	...
	/**
	 * Registers BP Dokumente's post types and taxonomies
	 *
	...
	function register_post_type() {
		...
		$post_type_labels = array(
			'name' 		     => _x( 'Dokumente', 'post type general name', 'bp-docs' ),
			'singular_name'      => _x( 'Dokument', 'post type singular name', 'bp-docs' ),
			'add_new' 	     => _x( 'Neues Dokument', 'add new', 'bp-docs' ),
			'add_new_item' 	     => __( 'Neues Dokument hinzufügen', 'bp-docs' ),
			'edit_item' 	     => __( 'Dokument bearbeiten', 'bp-docs' ),
			'new_item' 	     => __( 'Neues Dokument', 'bp-docs' ),
			'view_item' 	     => __( 'Dokument anzeigen', 'bp-docs' ),
			'search_items' 	     => __( 'Dokumente suchen', 'bp-docs' ),
			'not_found' 	     =>  __( 'Keine Dokumente gefunden', 'bp-docs' ),
			'not_found_in_trash' => __( 'Keine Dokumente im Papierkorb gefunden', 'bp-docs' ),
			'parent_item_colon'  => ''
		);
		...
		$associated_item_labels = array(
			'name'          => __( 'Zugehörige Elemente', 'bp-docs' ),
			'singular_name' => __( 'Zugehöriger Artikel', 'bp-docs' )
		);
	...
	function load_doc_extras() {
		...
		/**
		 * Should we apply Akismet filtration to BP Dokumente content?
		 *
		 * @since 2.1.0
		 *
	...
	function protect_doc_access() {
		...
		if ( ! is_user_logged_in() ) {
			...
			bp_core_add_message( __( 'Du hast keine Erlaubnis das zu tun.', 'bp-docs' ), 'error' );
